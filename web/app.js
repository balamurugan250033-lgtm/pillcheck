...
function getDoseSlots(profile) {
  if (Array.isArray(profile?.slots) && profile.slots.length) {
...
function renderMedicationProfile() {
  ...
  if (nextDoseTime) {
    nextDoseTime.innerHTML = `${time.time} <span>${time.period}</span>`;
  }
...
function renderSchedule(profile) {
  ...
  const slots = getDoseSlots(profile);
  list.innerHTML = `<div class="day-divider"><span>${profile.medicine} · ${profile.days} days</span><span>${slots.length} dose${slots.length === 1 ? '' : 's'} daily</span></div>${slots.map((slot, index) => {
    const time = formatDoseTime(slot.time);
    return `<div class="schedule-item ${index === 0 ? 'next' : ''}"><span class="schedule-time">${time.time} ${time.period}</span><div class="med-pill blue"></div><div><h3>${profile.medicine} <small>${profile.dose || 'Dose not specified'} · ${slot.food}</small></h3></div>${index === 0 ? '<button class="scan-button" id="scheduleScanBtn">Scan tablet <span>→</span></button>' : '<span class="state-badge upcoming">Upcoming</span>'}</div>`;
  }).join('')}${profile.issue ? `<p class="muted">Health issue: ${profile.issue}</p>` : ''}`;
...
function loadMedicationProfile() {
  ...
  document.querySelectorAll('input[name="doseSlot"]').forEach(input => {
    input.checked = getDoseSlots(profile).some(slot => slot.name === input.value);
  });
...
document.querySelector('#profileForm').addEventListener('submit', event => {
  ...
  const profile = {
    ...
    slots: [...document.querySelectorAll('input[name="doseSlot"]:checked')].map(input => ({
      name: input.value,
      time: document.querySelector(`#${input.value}Time`).value,
      food: document.querySelector(`#${input.value}Food`).value,
    })),
  };
...
fetch('./drug-database.json')
  .then(response => {
    if (!response.ok) {
      throw new Error('Drug database request failed');
    }
...
function openDatabase() {
  return new Promise((resolve, reject) => {
    const request = window.indexedDB.open('pillcheck-local', 1);
...
function matchDrugName(ocrText, expectedName) {
  ...
  const drugEntry = drugDatabase.find(item => item.name === expectedName);
  const candidates = [expected, ...(drugEntry ? drugEntry.aliases : [])].map(normalize);
...
function listenForCommand(onCommand) {
  ...
  recognition.onerror = event => {
    notify(event.error === 'not-allowed' ? 'Microphone permission was blocked. Allow it in your browser.' : 'I could not hear that. Please try again.');
  };
  recognition.onresult = event => {
    onCommand(event.results[0][0].transcript.toLowerCase());
  };
  recognition.onend = () => {
    recognition = null;
  };
...
async function continuousScan() {
  while (isScanning && !isScanResultVisible) {
    try {
      if (window.Tesseract && cameraStream && cameraVideo.videoWidth) {
        const frame = captureCameraFrame();
        const result = await window.Tesseract.recognize(frame, 'eng');
        const ocrText = result.data.text;
        const match = matchDrugName(ocrText, getMedicationProfile()?.medicine || 'Paracetamol');
        if (match.matched && match.score > 0.8) {
          showScanResult(false, ocrText, match);
          break;
        }
      }
    } catch (e) {
      // Quiet fail for background scan
    }
...
document.querySelectorAll('.scan-mode').forEach(mode =>
  mode.addEventListener('click', () => {
    ...
    const modeCopy = {
      pill: ['VERIFYING DOSE', 'Scan your medication', 'Hold one tablet steady in front of your camera. PillCheck checks shape, color, and imprint on-device.', 'Scan tablet', 'Place one tablet inside the frame'],
...
function handleVoiceCommand(command) {
  if (command.includes('scan')) {
    ...
    const btn = document.querySelector('#confirmDose');
    if (btn) {
      btn.click();
    }
...
async function showScanResult(forceMismatch = false, preDetectedOcr = '', preDetectedMatch = null) {
  if (isScanResultVisible) {
    return;
  }
  isScanResultVisible = true;
  let match = preDetectedMatch || { matched: false, score: 0, detected: 'no medicine name detected' };
  let ocrText = preDetectedOcr;

  if (!forceMismatch && !preDetectedMatch) {
    try {
      ...
      const result = await window.Tesseract.recognize(frame, 'eng', {
        logger: message => {
          if (message.status === 'recognizing text') {
            cameraStatus.textContent = `Reading image ${Math.round(message.progress * 100)}%`;
          }
        },
      });
    ...
    document.querySelector('#confirmDose').addEventListener('click', async () => {
      try {
        await saveAdherenceRecord({
          ...
          medicine: match.detected,
          expected: expectedMedicine,
          ocrText,
          score: match.score,
          result: 'match',
...
document.querySelector('#voiceBtn').addEventListener('click', () => {
  ...
  listenForCommand(command => {
    if (command.includes('scan')) {
      ...
      const profile = getMedicationProfile();
      speak(`Your next dose is ${profile?.medicine || 'your prescribed medication'}.`);
    } else if (command.includes('later') || command.includes('snooze')) {
      notify('Reminder snoozed for 15 minutes.');
    } else {
      notify(`I heard “${command}”. Try saying scan now or repeat.`);
    }
  });
});
...
document.querySelectorAll('.toggle.on').forEach(toggle => {
...
async function answerAdvisor(question) {
  ...
  try {
    ...
    const result = await response.json();
    if (!response.ok) {
      throw new Error(result.error || 'Advisor unavailable');
    }
    if (bubble) {
...
document.querySelectorAll('.suggestion').forEach(button => {
  button.addEventListener('click', () => {
    answerAdvisor(button.textContent);
  });
});
...
document.querySelector('#shiftReminder').addEventListener('click', () => {
  notify('Suggestion saved: move your evening reminder to 7:45 PM.');
});
document.querySelector('#advisorVoice').addEventListener('click', () => {
  notify('Listening for your health question…');
  if ('speechSynthesis' in window) {
    window.speechSynthesis.speak(new window.SpeechSynthesisUtterance('I am listening. Ask me about your adherence.'));
  }
});
