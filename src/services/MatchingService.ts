export function fuzzyMatchScore(expected: Record<string, string | undefined>, detected: Record<string, string | undefined>): number {
  ...
  keys.forEach(k => {
    if (expected[k] && detected[k]) {
      total += 2;
      if (normalize(expected[k]) === normalize(detected[k])) matches += 2;
      else if ((expected[k] || '').includes(detected[k] || '') || (detected[k] || '').includes(expected[k] || '')) matches += 1;
    } else if (expected[k] || detected[k]) {
      total += 1;
      if (!expected[k] && !detected[k]) matches += 1;
    }
  });
